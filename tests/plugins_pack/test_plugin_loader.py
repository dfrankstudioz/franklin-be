...
ROOT = Path(__file__).resolve().parents[2]   # repo root (…/docker)
CANDIDATES = [
    ROOT / "plugins",
    Path.cwd() / "plugins",
...
def test_plugins_are_loadable():
    ...
    # try importing at least one module if present
    samples = list(plugdir.glob("*.py"))
    if samples:
        modname = f"plugins.{samples[0].stem}"
