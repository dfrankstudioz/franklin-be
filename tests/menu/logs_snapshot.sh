...
TARGET_DIR="$HOME/docker/tests/active/logs"
if [[ -n "$PYTEST_BIN" && -d "$TARGET_DIR" ]] && compgen -G "$TARGET_DIR/test_*.py" > /dev/null; then
  echo "Running Logs & Snapshot Utilities via: $PYTEST_BIN -vv -s $TARGET_DIR"
  "$PYTEST_BIN" -vv -s "$TARGET_DIR"
else
  echo "No Logs & Snapshot Utilities found in $TARGET_DIR — placeholder OK."
  exit 0
fi
