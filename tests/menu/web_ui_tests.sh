...
TARGET_DIR="$HOME/docker/tests/active/web_ui"
if [[ -n "$PYTEST_BIN" && -d "$TARGET_DIR" ]] && compgen -G "$TARGET_DIR/test_*.py" > /dev/null; then
  echo "Running Web UI Tests via: $PYTEST_BIN -vv -s $TARGET_DIR"
  "$PYTEST_BIN" -vv -s "$TARGET_DIR"
else
  echo "No Web UI Tests found in $TARGET_DIR — placeholder OK."
  exit 0
fi
